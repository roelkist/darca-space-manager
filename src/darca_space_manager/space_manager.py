...
Supports hierarchical space creation, deletion, metadata tracking, and recursive discovery.
"""
...
INDEX_FILE = os.path.join(config.DIRECTORIES["METADATA_DIR"], "spaces_index.yaml")
...
class SpaceManager:
    ...
    def _load_index(self) -> Dict:
        try:
            ...
            raise SpaceManagerException("Failed to load spaces index.", error_code="INDEX_LOAD_FAILED", cause=e)
    ...
    def _save_index(self):
        try:
            ...
            raise SpaceManagerException("Failed to save spaces index.", error_code="INDEX_SAVE_FAILED", cause=e)
    ...
    def _scan_directory(self, directory: str) -> List[dict]:
        ...
        try:
            all_files = DirectoryUtils.list_directory(directory, recursive=True)
            metadata_files = [f for f in all_files if os.path.basename(f) == METADATA_FILENAME]

            for rel_path in metadata_files:
                full_path = os.path.join(directory, rel_path)
                try:
                    metadata = YamlUtils.load_yaml_file(full_path)
                    if all(k in metadata for k in ["name", "label", "path", "created_at"]):
                        discovered.append(metadata)
                        logger.debug(f"🔎 Discovered valid space: {metadata['name']} at {metadata['path']}")
                    else:
                        logger.warning(f"⚠️ Incomplete metadata in {full_path}, skipping.")
                except Exception as e:
                    logger.warning(f"⚠️ Failed to load metadata in {full_path}: {e}")
        except DirectoryUtilsException as e:
            raise SpaceManagerException("Failed to scan directory.", error_code="DIRECTORY_SCAN_FAILED", cause=e)
        except Exception as e:
            raise SpaceManagerException("Unexpected error during directory scan.", cause=e)
    ...
    def get_space(self, name: str) -> Union[dict, None]:
        try:
            return next((s for s in self.index["spaces"] if s["name"] == name), None)
        except Exception as e:
            logger.error(f"❌ Failed to get space '{name}'.", exc_info=True)
            raise SpaceManagerException("Failed to retrieve space.", metadata={"space": name}, cause=e)

    def create_space(self, name: str, label: str = "", parent_path: str = None) -> bool:
        """
        ...
        Args:
            ...
            parent_path (str): A path like 'space1/subdir', where the first part must be an existing space.
        ...
        try:
            ...
            if parent_path:
                ...
                if not base_space:
                    raise SpaceManagerException(
                        f"Base space '{base_space_name}' not found in path '{parent_path}'.",
                ...
                if not destination_path.startswith(base_path):
                    raise SpaceManagerException(
                        ...
                        metadata={"base": base_space_name, "resolved_path": destination_path},
                    )
            ...
            # Write metadata
            metadata = self._generate_metadata(name=name, label=label, path=destination_path)
            ...
            logger.error(f"❌ Failed to create space '{name}' under '{parent_path}'.", exc_info=True)
            raise SpaceManagerException(
                message=f"Failed to create space '{name}' under '{parent_path}'.",
    ...
    def delete_space(self, name: str) -> bool:
        ...
        if not space:
            raise SpaceManagerException(f"Space '{name}' not found.", metadata={"space": name})
        ...
        try:
            ...
            raise SpaceManagerException(f"Failed to delete space '{name}'.", metadata={"space": name}, cause=e)

    def list_spaces(self, label_filter: str = None) -> List[dict]:
        try:
            logger.debug(f"📃 Listing spaces (filter: {label_filter})")
            return [s for s in self.index["spaces"] if s.get("label") == label_filter] if label_filter else self.index["spaces"]
        except Exception as e:
            logger.error("❌ Failed to list spaces.", exc_info=True)
            raise SpaceManagerException("Failed to list spaces.", error_code="LIST_SPACES_FAILED", cause=e)

    def create_directory(self, space_name: str, relative_path: str) -> str:
        space = self.get_space(space_name)
        if not space:
            raise SpaceManagerException("Space not found for directory creation.", metadata={"space": space_name})

        abs_path = os.path.normpath(os.path.join(space["path"], relative_path))
        if not abs_path.startswith(space["path"]):
            raise SpaceManagerException("Directory path escapes space boundaries.", metadata={"space": space_name})

        try:
            DirectoryUtils.create_directory(abs_path)
            logger.info(f"📁 Created directory '{relative_path}' in space '{space_name}'.")
            return abs_path
        except Exception as e:
            logger.error(f"❌ Failed to create directory in space '{space_name}'.", exc_info=True)
            raise SpaceManagerException("Failed to create directory.", metadata={"space": space_name, "path": relative_path}, cause=e)

    def remove_directory(self, space_name: str, relative_path: str) -> bool:
        space = self.get_space(space_name)
        if not space:
            raise SpaceManagerException("Space not found for directory removal.", metadata={"space": space_name})
        ...
        try:
            ...
            logger.info(f"🗑️ Removed directory '{relative_path}' in space '{space_name}'.")
            return True
        except Exception as e:
            logger.error(f"❌ Failed to remove directory in space '{space_name}'.", exc_info=True)
            raise SpaceManagerException("Failed to remove directory.", metadata={"space": space_name, "path": relative_path}, cause=e)
