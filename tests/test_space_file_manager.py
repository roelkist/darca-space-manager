...
def test_get_file_last_modified_success(space_file_manager, space_manager):
    ...
    reported_mtime = space_file_manager.get_file_last_modified(space_name, file_name)

    assert isinstance(reported_mtime, float), "Returned timestamp should be a float"
    assert abs(reported_mtime - actual_mtime) < 0.0001, (
        "Reported timestamp should be within a tiny delta of the actual filesystem mtime."
    )


def test_get_file_last_modified_nonexistent_file(space_file_manager, space_manager):
    """
    If the file doesn't exist, we expect a SpaceFileManagerException with 'FILE_NOT_FOUND'.
    """
    space_name = "mtime_file_missing"
    space_manager.create_space(space_name)
    
    with pytest.raises(SpaceFileManagerException) as exc_info:
        space_file_manager.get_file_last_modified(space_name, "no_such_file.txt")
...
def test_get_file_last_modified_os_error(space_file_manager, space_manager):
    ...
    with patch("os.path.getmtime", side_effect=OSError("Mocked getmtime error")):
        with pytest.raises(SpaceFileManagerException) as exc_info:
