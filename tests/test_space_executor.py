...
def test_run_in_space_command_failure(space_executor, space_manager):
    """
    If the command fails (non-zero exit code), DarcaExecError is raised in DarcaExecutor,
    which is then caught and re-raised as SpaceExecutorException.
    """
    space_name = "executor_failure_space"
    space_manager.create_space(space_name)

    # Command that should fail with a non-zero exit code (ls on a non-existent dir).
    with pytest.raises(SpaceExecutorException) as exc_info:
        space_executor.run_in_space(space_name, ["ls", "non_existent_directory"])
    assert "Failed to run command in space" in str(exc_info.value)
    # We also expect the re-wrapped metadata, including 'returncode', 'stdout', 'stderr'.
...
def test_run_in_space_unexpected_exception(space_executor, space_manager):
    ...
    with patch.object(space_executor._executor, "run", side_effect=ValueError("Mocked error")):
        with pytest.raises(SpaceExecutorException) as exc_info:
            space_executor.run_in_space(space_name, ["echo", "test"])
    assert "Unexpected error while running command in space" in str(exc_info.value)
    assert "VALUEERROR" not in str(exc_info.value).upper()  # It's re-wrapped, but you can check cause if needed
